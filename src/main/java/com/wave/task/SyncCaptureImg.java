 ...
 * @version V1.0
 */
public class SyncCaptureImg extends SyncCaptureBase implements Job {

	private final static Logger LOG = Logger.getLogger(SyncCaptureImg.class);
	
	private BlockingQueue<CaptureImg> queue;
	
	private ReentrantLock lock;

	@Override
	public void execute(JobExecutionContext context) throws JobExecutionException {
		setTable(Constant.IMG_TABLE_NAME);
		...
		queue = new ArrayBlockingQueue<CaptureImg>(1024);
		Config config = LoadConfig.getInstance("task");

		record = getRecord();
		current = Util.getCurrentRecord();
		LOG.info(table + " : 启动读取pg库数据线程...");
		CaptureImg empty = new CaptureImg(true);
		new QueryThread<CaptureImg>(queue, empty).start();
		...
		for (int index = 0; index < writes.length; index++) {
			...
			writes[index] = new WritesThread<CaptureImg>(queue, lock);
